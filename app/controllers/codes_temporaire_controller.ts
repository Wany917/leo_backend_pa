...
export default class CodeTemporairesController {
  async generate_code({ request, response }: HttpContext) {
    ...
    const userExists = await CodeTemporaire.query().where('user_info', user_info).first()
  ...
  async check_code({ request, response }: HttpContext) {
    try {
      ...
      if (codeTemporaire) {
        await CodeTemporaire.query().where('user_info', user_info).where('code', code).delete()
  ...
  async reset_code({ request, response }: HttpContext) {
    try {
      const { user_info } = await request.validateUsing(generateCodeValidator)
      const codeTemporaire = await CodeTemporaire.query().where('user_info', user_info).first()

      if (codeTemporaire) {
        const code = Math.floor(100000 + Math.random() * 900000).toString()
        await CodeTemporaire.query().where('user_info', user_info).update({ code: code })
